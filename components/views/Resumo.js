...
export default function Resumo({ ir }) {
  ...
  const totalBRL = gastos.reduce((s, g) => s + valorEmBRL(g, cotacao), 0);
  ...
  const porPessoa = perfis
    .map((p) => ({ ...p, total: gastos.filter((g) => g.pago_por === p.id).reduce((s, g) => s + valorEmBRL(g, cotacao), 0) }))
    .sort((a, b) => b.total - a.total);
  const maior = Math.max(1, ...porPessoa.map((p) => p.total));
  const ultimos = gastos.slice(0, 4);

  function editarCotacao() {
    const v = window.prompt('Cotação geral (R$ por US$ 1). Usada quando você não informar a cotação da compra.', String(cotacao));
    if (v != null) { const n = parseFloat(v.replace(',', '.')); if (!isNaN(n) && n > 0) atualizarCotacao(n); }
  }
    ...
    <div className="screen">
      <div className="pass">
        ...
        <div className="budget">
          ...
          <div className="meta">{pct}% usado · {restante >= 0 ? `${fmtBRL(restante)} restante` : `${fmtBRL(-restante)} acima`}</div>
        </div>
        <div className="perf" />
        <div className="split">
          ...
          <div className="col" onClick={editarCotacao} style={{ cursor: 'pointer' }}><div className="k">Cotação geral</div><div className="v">R$ {cotacao.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div></div>
        </div>
      </div>
