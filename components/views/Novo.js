...
export default function Novo({ ir }) {
  ...
  // partes por pessoa: { [perfilId]: number }  (0 = não participa)
  const [partes, setPartes] = useState(() => {
    const init = {};
    perfis.forEach((p) => { init[p.id] = 1; });
    return init;
  });
  const [salvando, setSalvando] = useState(false);

  function togglePessoa(id) {
    setPartes((prev) => ({ ...prev, [id]: prev[id] > 0 ? 0 : 1 }));
  }
  function mudarPartes(id, delta) {
    setPartes((prev) => {
  ...
  const participantes = perfis
    .filter((p) => (partes[p.id] || 0) > 0)
    .map((p) => ({ id: p.id, partes: partes[p.id] }));

  const valido = valor && parseFloat(valor.replace(',', '.')) > 0 && pagoPor && participantes.length > 0;
      ...
      <div className="screen" style={{ paddingTop: 18 }}>
        ...
        <div className="field">
          ...
          {perfis.map((p) => {
            ...
            return (
              <div className="partes" key={p.id}>
                <button className={'chip' + (ativo ? ' on' : '')} onClick={() => togglePessoa(p.id)} style={{ minWidth: 120, textAlign: 'left' }}>
                  {p.nome}
                </button>
                {ativo && (
          ...
          <p style={{ fontSize: 11, color: 'var(--faint)', marginTop: 8 }}>
            Partes diferentes = divisão proporcional. Ex.: 2 e 1 = um paga o dobro do outro.
          </p>
        </div>

        <button className="btn-primary" onClick={salvar} disabled={!valido || salvando}>
...
function nomeDe(catId) {
  const c = CATEGORIAS.find((x) => x.id === catId);
  return c ? c.nome : 'Gasto';
}
