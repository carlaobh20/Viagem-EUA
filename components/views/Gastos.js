...
import { fmtUSD, fmtBRL, emojiCategoria, nomeCategoria } from '../../lib/format';

export default function Gastos() {
  const { gastos, perfis, divisoes, removerGasto } = useData();
  const [filtro, setFiltro] = useState('todos'); // todos | USD | BRL

  const lista = gastos.filter((g) => filtro === 'todos' || g.moeda === filtro);

  function nomePagador(id) {
    const p = perfis.find((x) => x.id === id);
    return p ? p.nome : '—';
  }
  function qtdParticipantes(gid) {
    return divisoes.filter((d) => d.gasto_id === gid).length;
  }
    ...
    <div className="screen">
        ...
        {lista.map((g) => (
          <div className="row" key={g.id} onDoubleClick={() => confirmarRemover(g, removerGasto)}>
            ...
            <span className="meta">
              ...
              <span className="s">
                {nomePagador(g.pago_por)} · dividido entre {qtdParticipantes(g.id)} · {formataData(g.data)}
              </span>
            </span>
      ...
      {lista.length > 0 && (
        <p style={{ fontSize: 11, color: 'var(--faint)', textAlign: 'center', marginTop: 12 }}>
          Toque duas vezes em um gasto para apagar.
        </p>
      )}
    </div>
  );
...
function confirmarRemover(g, removerGasto) {
  if (window.confirm(`Apagar o gasto "${g.descricao}"?`)) removerGasto(g.id);
}
