  ...
  return (
    <div className="screen">
      <div className="chips" style={{ marginBottom: 16 }}>
        {['todos', 'USD', 'BRL'].map((f) => (
          <button key={f} className={'chip' + (filtro === f ? ' on' : '')} onClick={() => setFiltro(f)}>{f === 'todos' ? 'Todos' : f === 'USD' ? 'Dólar' : 'Real'}</button>
        ))}
      </div>
      <div className="card">
        ...
        {lista.map((g) => (
          <div className="row" key={g.id}>
            ...
            <span className="meta"><span className="t">{g.descricao || nomeCategoria(g.categoria)}</span><span className="s">{nomePagador(g.pago_por)} · dividido entre {qtd(g.id)} · {formataData(g.data)}</span></span>
