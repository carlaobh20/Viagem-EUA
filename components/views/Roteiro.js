...
export default function Roteiro({ ir }) {
  ...
  const gastoDoPonto = (id) =>
    gastos.filter((g) => g.ponto_id === id).reduce((s, g) => s + paraBRL(g.valor, g.moeda, cotacao), 0);
  ...
  async function adicionarParada() {
    ...
    await supabase.from('pontos_roteiro').insert({
      viagem_id: viagem.id,
      nome,
    ...
    <div className="app">
      <div className="screen" style={{ paddingTop: 18 }}>
        <div className="fab-back">
          ...
          <span className="ttl">Roteiro</span>
        </div>

        <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 18 }}>
          Pontos de parada da viagem com o gasto de cada lugar.
        </p>

        {pontos.length === 0 ? (
          <div className="card"><div className="empty">Nenhuma parada ainda.</div></div>
        ) : (
        ...
        <button className="btn-primary" style={{ background: 'var(--surface)', color: 'var(--brand)', border: '0.5px solid var(--brand)' }} onClick={adicionarParada}>
          + Adicionar parada
        </button>
      </div>
    </div>
...
function periodo(p) {
  if (!p.data_inicio) return 'sem data';
  const f = (d) => {
    const [, m, dia] = d.split('-');
    const meses = ['jan', 'fev', 'mar', 'abr', 'mai', 'jun', 'jul', 'ago', 'set', 'out', 'nov', 'dez'];
    return `${dia} ${meses[Number(m) - 1]}`;
  };
  return p.data_fim ? `${f(p.data_inicio)} – ${f(p.data_fim)}` : f(p.data_inicio);
}
